...
describe('Register', () => {
  it('can not register test domain if already owned', () => {
    ...
    cy.queryByText('Transfer', { exact: false, timeout: 100 }).should('exist')
    cy.queryByText('Not owned yet', { exact: false, timeout: 100 }).should(
      'not.exist'
    )
  ...
  it('can register test domain', () => {
    ...
    cy.getByText('Claim the test domain').click()
    cy.wait(1000)
    cy.queryByText('Not owned yet', { exact: false, timeout: 10000 }).should(
      'not.exist'
    )
